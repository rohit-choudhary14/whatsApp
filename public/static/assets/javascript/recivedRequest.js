export function initRecevedRequest(closeLoader) {
  async function recivedRequest() {
    try {
      fetch("/api/recivedRequestList", {
        method: "GET",
        headers: {
        ...
        .then((data) => {
          ...
          let parent = document.getElementById("friend_list_parent_container");
          parent.innerHTML = "";
          let count = 0;
          result.forEach((element) => {
            ...
            newDiv.style.boxShadow = "0px 0px 3px rgb(0 0 0 / 24%)";
            ...
            newDiv.innerHTML = `
                    ...
                    <div>
                      <p style='margin-top: 5px;margin-left:0px;word-wrap: unset;overflow: hidden;max-width: 110px;text-wrap: nowrap;'>
                      ${element.sender_user_name}
                      </p>
                    </div>
                    <div>
                      <button onclick="accept_request('${id}')"   style='border: none;background: #008069;color: white;padding: 0.3rem;border-radius: 4px;' id='accept_request' >${button}</button>

                    </div>
                      <div>
                      <button data-sender-mobile-number=${element.sender_mobile_number}  data-cancel-button=${element.reciver_user_id} id='remove_friend_request' onclick='remove_friend_request(${count})' style='border: none;background: #757575;color: white;padding: 0.3rem;border-radius: 4px;'>${cancel}</button>
